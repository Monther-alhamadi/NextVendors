...
def validate_email_local_part(
    ...
    bad_chars = set(
        safe_character_display(c) for c in local if not ATEXT_INTL_RE.match(c)
    )
    if bad_chars:
        raise EmailSyntaxError(
...
def validate_email_domain_literal(domain_literal, allow_domain_literal=False):
    ...
    if re.match(r"^[0-9\.]+$", domain_literal):
        ...
        if not allow_domain_literal:
            raise EmailSyntaxError(
                "A bracketed IPv4 address after the @-sign is not allowed here."
            )
    ...
    if domain_literal.startswith("IPv6:"):
        ...
        if not allow_domain_literal:
            raise EmailSyntaxError(
                "A bracketed IPv6 address after the @-sign is not allowed here."
            )
