...
def generate_latest(registry, escaping=UNDERSCORES, version="1.0.0"):
    ...
    for metric in registry.collect():
        try:
            ...
            output.append(
                f'# TYPE {escape_metric_name(mname, escaping)} {metric.type}\n'
            )
            if metric.unit:
                output.append(
                    f'# UNIT {escape_metric_name(mname, escaping)} {metric.unit}\n'
                )
            for s in metric.samples:
                ...
                if s.native_histogram:
                    ...
                    if s.native_histogram.neg_spans:
                        ...
                        negative_deltas = ','.join(
                            str(nd) for nd in s.native_histogram.neg_deltas
                        )
                    ...
                    if s.native_histogram.pos_spans:
                        ...
                        positive_deltas = ','.join(
                            f'{pd}' for pd in s.native_histogram.pos_deltas
                        )
