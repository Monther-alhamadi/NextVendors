...
def generate_latest(
    ...
    def sample_line(samples):
        ...
        if (
            escaping != openmetrics.ALLOWUTF8
            or openmetrics._is_valid_legacy_metric_name(samples.name)
        ):
            if labelstr:
    ...
    for metric in registry.collect():
        try:
            ...
            output.append(
                f'# TYPE {openmetrics.escape_metric_name(mname, escaping)} {mtype}\n'
            )
    ...
    for accepted in accept_header.split(','):
        if accepted.split(';')[0].strip() == 'application/openmetrics-text':
            ...
            if version and parse_version(version) >= (1, 0, 0):
                return (
                    partial(
                        openmetrics.generate_latest, escaping=escaping, version=version
                    ),
