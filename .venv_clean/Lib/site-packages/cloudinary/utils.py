...
def generate_transformation_string(**options):
    ...
    if variables:
        for var in variables:
            var_params.append(
                u"{0}_{1}".format(var[0], normalize_expression(str(var[1])))
            )
...
def sign_request(params, options):
    ...
    signature_version = options.get(
        "signature_version", cloudinary.config().signature_version
    )

    params = cleanup_params(params)
    params["signature"] = api_sign_request(
        params, api_secret, signature_algorithm, signature_version
    )
...
def api_sign_request(
    params_to_sign, api_secret, algorithm=SIGNATURE_SHA1, signature_version=2
):
    """
