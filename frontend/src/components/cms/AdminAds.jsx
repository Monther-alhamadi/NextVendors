...
export default function AdminAds() {
  ...
  const ZONES = [
    { value: "homepage_top", label: t('admin.ads.zones.home_top', 'أعلى الصفحة الرئيسية') },
    { value: "homepage_mid", label: t('admin.ads.zones.home_mid', 'منتصف الصفحة الرئيسية') },
    { value: "product_list", label: t('admin.ads.zones.product_list', 'قائمة المنتجات') },
    { value: "sidebar", label: t('admin.ads.zones.sidebar', 'الشريط الجانبي') },
    { value: "checkout_bottom", label: t('admin.ads.zones.checkout_bottom', 'أسفل صفحة الدفع') },
  ];

  const AD_TYPES = [
    { value: "image", label: t('common.image', 'صورة') },
    { value: "html", label: t('common.html_code', 'كود HTML مخصص') },
  ];
  ...
  async function handleSave() {
    if (!form.title.trim()) {
      toast.push({ message: t("admin.ads.title_req"), type: "warning" });
      return;
    }
    setSaving(true);
    try {
      ...
      if (editingAd) {
        ...
        toast.push({ message: t("admin.ads.update_success"), type: "success" });
      } else {
        await createAd(payload);
        toast.push({ message: t("admin.ads.create_success"), type: "success" });
      }
      closeModal();
      await loadAds();
    } catch (e) {
  ...
  async function handleDelete(id) {
    try {
      await deleteAd(id);
      toast.push({ message: t("admin.ads.delete_success"), type: "success" });
    ...
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <button className={styles.btnPrimary} onClick={openCreate}>
          <PlusCircle size={16} /> {t("admin.ads.new_ad", "إنشاء إعلان جديد")}
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className={styles.emptyState}>
          ...
          <p>{t("admin.ads.empty", "لا توجد إعلانات بعد.")}</p>
          <button className={styles.btnPrimary} onClick={openCreate}><PlusCircle size={16} /> {t("admin.ads.create")}</button>
        </div>
      ) : (
        <div className={styles.tableWrapper}>
          <table className={styles.table}>
            <thead>
              <tr>
                <th>{t("admin.ads.col_title")}</th>
                <th>{t("admin.ads.col_zone")}</th>
                <th>{t("common.type")}</th>
                <th>{t("admin.ads.col_status")}</th>
                ...
                <tr key={ad.id} className={!ad.is_active ? styles.rowInactive : ""}>
                  ...
                  <td>
                    <span className={styles.typeBadge}>
                      ...
                      {ad.content?.type === "html" ? t("common.html") : t("common.image")}
                    </span>
                  </td>
                  <td>
                    <button className={`${styles.toggleBtn} ${ad.is_active ? styles.on : styles.off}`} onClick={() => handleToggle(ad)}>
                      ...
                      <span>{ad.is_active ? t("common.active") : t("common.inactive")}</span>
                    </button>
                  </td>
                  <td>
                    <div className={styles.actions}>
                      <button className={styles.btnEdit} onClick={() => openEdit(ad)} title={t("common.edit")}><Edit2 size={15} /></button>
                      <button className={styles.btnDelete} onClick={() => setDeleteConfirm(ad.id)} title={t("common.delete")}><Trash2 size={15} /></button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              ...
              {form.content.type === "image" ? (
                <>
                  <div className={styles.fieldGroup}>
                    <label>{t("admin.ads.img_url")}</label>
                    <input className={styles.input} value={form.content.url} onChange={e => setContentField("url", e.target.value)} />
                  </div>
                  <div className={styles.fieldGroup}>
                    <label>{t("admin.ads.link")}</label>
                    <input className={styles.input} value={form.content.link} onChange={e => setContentField("link", e.target.value)} />
                  </div>
                  {form.content.url && (
                    <div className={styles.preview}>
                      <p className={styles.previewLabel}>{t("common.preview", "معاينة:")}</p>
                ...
                <div className={styles.fieldGroup}>
                  <label>{t("admin.ads.html_code")}</label>
                  <textarea className={styles.textarea} rows={6} value={form.content.html_code} onChange={e => setContentField("html_code", e.target.value)} />
                </div>
              )}

              <div className={styles.row2}>
                <div className={styles.fieldGroup}>
                  <label>{t("admin.ads.start_date", "تاريخ البدء (اختياري)")}</label>
                  <input className={styles.input} type="datetime-local" value={form.start_at} onChange={e => setForm(f => ({ ...f, start_at: e.target.value }))} />
                </div>
                <div className={styles.fieldGroup}>
                  <label>{t("admin.ads.end_date", "تاريخ الانتهاء (اختياري)")}</label>
              ...
              <label className={styles.checkRow}>
                ...
                <span>{t("admin.ads.activate_now")}</span>
              </label>
              </div>
            </div>
            <div className={styles.modalFooter}>
              ...
              <button className={styles.btnPrimary} onClick={handleSave} disabled={saving}>
                <Save size={16} />{saving ? t("common.saving") : t("admin.ads.save")}
              </button>
            </div>
          </div>
        </div>
